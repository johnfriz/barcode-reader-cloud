      ...
      if(err || !cacheRes) {
        ...
        fh.service({guid : services.barcode, path : '/barcode/read', method : 'GET',params : {barcode : barcodeId}},
          function(err, body, barcodeServiceRes) {
            logger.debug('Res Status Code = ',  barcodeServiceRes.statusCode);
            if (err || barcodeServiceRes.statusCode != 200) {
              logger.warn('Error calling barcode service - ' + err);
              return res.status(500).send({"error":"Unable to reach barcode microservice"});
            } else {
